# 12 · Ad Packages — Copy (DE / EN)

> Fertige Texte für die Pricing-Sektion der Monatsabos, in Deutsch und Englisch.
> Preise: Ad Starter $197 · Ad Premium $497 · Ad Agency $997 (USD/Monat).
> Platzhalter in `{{...}}`.

---

## 🇩🇪 Deutsch

### Sektion
**Headline:** Monatliche Ad-Pakete
**Subheadline:** Frische Amazon-Werbe-Creatives & Ad-Videos – jeden Monat, ohne eigenen Designer.
**Hinweis:** Monatlich abgerechnet · jederzeit zum Monatsende kündbar · jährlich zahlen = 2 Monate gratis.

### Tarif 1 — Ad Starter · $197/Monat
- **Tagline:** Für den Einstieg
- **Beschreibung:** Perfekt, um konstant frische Werbemittel zu testen.
- **Enthalten:**
  - 2 Brand Ad Creatives / Monat
  - 1 Ad Video / Monat
  - Amazon-optimierte Lieferung
- **Für wen:** Einzelne Seller
- **CTA:** Abo starten

### Tarif 2 — Ad Premium · $497/Monat · ⭐ POPULAR
- **Tagline:** Der Bestseller
- **Beschreibung:** Genug Output, um deine wichtigsten Produkte konstant frisch und konversionsstark zu halten.
- **Enthalten:**
  - 5 Brand Ad Creatives / Monat
  - 3 Ad Videos / Monat
  - Amazon-optimierte Lieferung
  - Monatlich kündbar
- **Für wen:** Wachsende Marken
- **CTA:** Abo starten

### Tarif 3 — Ad Agency · $997/Monat
- **Tagline:** Für Agenturen & Multi-Brand
- **Beschreibung:** Hohes Volumen für mehrere Marken oder Kunden – inklusive White-Label-Dateien.
- **Enthalten:**
  - 12 Brand Ad Creatives / Monat
  - 8 Ad Videos / Monat
  - Priority-Lieferung
  - White-Label-Dateien
- **Für wen:** Agenturen
- **CTA:** Abo starten

### Microcopy (UI)
- Verbrauch: „Diesen Monat: {{x}} von {{y}} Creatives · {{a}} von {{b}} Videos übrig"
- Erneuerung: „Erneuert sich am {{datum}}"
- Aufgebraucht: „Monats-Kontingent aufgebraucht. Neues Guthaben am {{datum}}."
- Verwalten: „Abo verwalten / kündigen"
- Gekündigt: „Aktiv bis {{datum}} – danach endet dein Abo."
- Zahlung fehlgeschlagen: „Zahlung fehlgeschlagen – bitte aktualisiere deine Zahlungsmethode."

---

## 🇬🇧 English

### Section
**Headline:** Monthly Ad Packages
**Subheadline:** Fresh Amazon ad creatives & videos — every month, no designer needed.
**Note:** Billed monthly · cancel anytime, effective end of period · pay yearly = 2 months free.

### Tier 1 — Ad Starter · $197/mo
- **Tagline:** Get started
- **Description:** Perfect for consistently testing fresh ad creative.
- **What's included:**
  - 2 Brand Ad Creatives / month
  - 1 Ad Video / month
  - Amazon-optimized delivery
- **Best for:** Solo sellers
- **CTA:** Subscribe

### Tier 2 — Ad Premium · $497/mo · ⭐ POPULAR
- **Tagline:** Most popular
- **Description:** Enough output to keep your key products consistently fresh and converting.
- **What's included:**
  - 5 Brand Ad Creatives / month
  - 3 Ad Videos / month
  - Amazon-optimized delivery
  - Cancel anytime
- **Best for:** Growing brands
- **CTA:** Subscribe

### Tier 3 — Ad Agency · $997/mo
- **Tagline:** For agencies & multi-brand
- **Description:** High volume across multiple brands or clients — including white-label files.
- **What's included:**
  - 12 Brand Ad Creatives / month
  - 8 Ad Videos / month
  - Priority delivery
  - White-label files
- **Best for:** Agencies
- **CTA:** Subscribe

### Microcopy (UI)
- Usage: "This month: {{x}} of {{y}} creatives · {{a}} of {{b}} videos left"
- Renewal: "Renews on {{date}}"
- Exhausted: "Monthly allowance used up. Resets on {{date}}."
- Manage: "Manage / cancel subscription"
- Canceled: "Active until {{date}} — your plan ends after that."
- Payment failed: "Payment failed — please update your payment method."

---

## Optional: Value-Prop-Block (über den Karten)

**DE:** Schluss mit teuren Einzelaufträgen und langsamen Designern. Hol dir jeden Monat
planbar frische, Amazon-optimierte Ad-Creatives und Videos – produziert von Designern + AI.

**EN:** No more pricey one-offs or slow designers. Get a predictable monthly supply of
fresh, Amazon-optimized ad creatives and videos — produced by designers + AI.
