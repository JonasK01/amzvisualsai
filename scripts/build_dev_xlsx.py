...
rows = [
 ...
 # --- SUBSCRIPTIONS (Ad packages, monthly) ---
 ("S1","High","Subscriptions","New","Stripe subscription + Customer Portal",
  "New monthly recurring plans parallel to one-time services (e.g. 'Ad Premium Package' = 5 brand ad creatives + 3 ad videos/month). Stripe Product + recurring monthly Price, Checkout in subscription mode. Enable Stripe Customer Portal for self-serve cancel (at period end), card update, invoices = near-zero dev.","decision 13.06","4"),
 ("S2","High","Subscriptions","New","Plan/quota data model (generic, multi-tier)",
  "Data-driven plans with monthly quotas JSON e.g. {ad_creative:5, ad_video:3}; subscriptions + per-period quota_usage tables tied to org_id. Build GENERIC (multiple tiers planned), not hardcoded to one package.","decision 13.06","4"),
 ("S3","High","Subscriptions","New","Subscription webhooks",
  "Handle checkout.session.completed/subscription.created (provision + set first-period quota), invoice.paid (RESET monthly quota = new period), subscription.updated (status, cancel_at_period_end, plan change), subscription.deleted, invoice.payment_failed (past_due + restrict). Verify signatures, idempotent.","decision 13.06","4"),
 ("S4","High","Subscriptions","New","Quota enforcement (use-it-or-lose-it)",
  "On each ad-creative/ad-video generation: if period used_count < limit -> allow + increment atomically, else BLOCK with reset date. No rollover, no overage (block until next cycle).","decision 13.06","4"),
 ("S5","Medium","Subscriptions","New","Plans UI + usage display",
  "New Plans/Subscription section (tier cards), Subscribe CTA. Current plan with remaining-this-month (e.g. 'Creatives 3/5, Videos 2/3'), renews date, Manage/Cancel -> Customer Portal. Disable generate + show reset date when exhausted; past_due banner.","decision 13.06","4"),
 ("S6","Low","Subscriptions","New","Upgrade/downgrade handling",
  "Mid-cycle plan change: Stripe handles billing proration; quota change applies from next period (v1). Refine later.","decision 13.06","later"),
